...
def upgrade (ipaddr):
    try:
        ...
        output_version = net_connect.send_command('show inventory', use_textfsm=True)
        net_connect.enable()
        for n in output_version:
            if n["pid"].startswith("SM-X"):
                print(f"Copying IOS on {n['pid']}")
